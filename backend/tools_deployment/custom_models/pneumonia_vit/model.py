"""
Pneumonia ViT Classifier
Pipeline: Image → ViT encoder (google/vit-base-patch16-224) → CLS token (768) → MLP head → 2 classes
...
class PneumoniaViT(nn.Module):
    ...
    def freeze_backbone(self):
        """Freeze ViT encoder — train only MLP head."""
...
def save_model(model: PneumoniaViT, output_dir: str):
    import os, json
...
def load_model(model_dir: str) -> PneumoniaViT:
    import os, json
    with open(os.path.join(model_dir, "config.json")) as f:
        config = json.load(f)
    model = PneumoniaViT(num_classes=config["num_classes"], mlp_hidden=config.get("mlp_hidden", 256))
