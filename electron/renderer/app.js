...
function renderViewButtons() {
  document.querySelectorAll('.pill[data-view]').forEach((button) => {
...
function upsertScreenshot(entry) {
...
async function handleDismissAndHide() {
...
function setupOrb() {
  ...
  document.querySelectorAll('.satellite').forEach((sat) => {
    sat.addEventListener('click', () => {
      state.activeView = sat.dataset.view;
      state.uiMode = 'bar';
      render();
    });
  });
...
function bindEvents() {
  ...
  elements.doctorInput.addEventListener('input', (event) => {
    state.doctorDraft = event.target.value;
    render();
  });

  // Flag input
  elements.flagInput.addEventListener('input', (event) => {
    state.flagDraft = event.target.value;
  });
  ...
  document.querySelectorAll('.toggle-pill').forEach((pill) => {
    pill.addEventListener('click', () => {
      state.inputMode = pill.dataset.mode;
...
async function bootstrap() {
  ...
  try {
    ...
    state.statusMessage = `Bridge unavailable: ${error.message}`;
  ...
  try {
    ...
    appendTranscript('system', `Live state unavailable: ${error.message}`);
  }

  try {
    connectEvents();
  } catch (error) {
    appendTranscript('system', `Event stream unavailable: ${error.message}`);
  }
  ...
  try {
    ...
    appendTranscript('system', `Live bootstrap failed: ${error.message}`);
  }
